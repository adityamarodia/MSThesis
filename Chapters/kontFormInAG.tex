...
We start with studying some moduli theory to understand the language used in Kontsevich's proof.
...
\par This chapter's exposition is majorly ispired from \cite{KockQcohom}. We also recommend the reader to refer to \cite{FPnotes} for details (and further generalisations) we don't discuss during the course of this chapter.
...
Before I state what exactly a Moduli problem entails, consider the following example:
...
Before we formally define a moduli problem in the context of categories let's revisit example \ref{exLinesInPlane}.
\begin{example}
...
We can interpret this reversing of directions as a contravariant functor (called the moduli functor) which sends each scheme to the equivalence class of families over it. 
...
\end{definition}
By Yoneda's Lemma (appendix) we know that we can identify the natiral transformation $\mathcal{U}:h_{Y}\to S$ with an element of $S(Y)$.
...
For a quedrupule of points $p_1, \dots, p_4$, let $\alpha$ be the automorphism determined by the first three points going to $0,\,1,\, \infty$. 
...
For a quadruple of points $(p_{1}, p_{2}, p_{3}, p_{4})$ let $\lambda((p_{1}, p_{2}, p_{3}, p_{4}))$ denote their cross ratio.
Then we can characterise projective equivalence of two $n$-tuples in the following manner.
...
If $p_{n+1}$ lies on a twig with $4$ or more special points, the resulting $n$ pointed curve remains stable, hence the resulting curve is just the same curve with one less point. 
...
\begin{remark}
    Since the set $S = \{p_{1}, \dots, p_{n}\}$ we partition to define cycles already has an ordering, we will assume that the ordering on the partitions is induced from this ordering. Hence we choose to drop the term \textit{ordered} while referring to partitions.
\end{remark}
...
The open set, $U$, set we are interested is the set given by maps in $\overline{\mathcal{M}}_{0,n}(\mathbb{P}^{2},d)$ such that their images intersect transversaly with the lines $l_{i}$.
